...
class ReplayerSpec(config: ReallyConfig) extends BaseActorSpec(config) {
  ...
  "Replayer with no max marker" should "get logs if matches the rev first then switch to serve push updates" in {
    val objectSubActor = TestProbe()
    val requestDelegate = TestProbe().ref
    val pushChannel = TestProbe().ref
    val rev: Revision = 1L
